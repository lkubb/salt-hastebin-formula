...
We will see a quick setup to configure the ``haste`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``haste`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``haste`` formulas under ``/srv/salt/haste/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/haste/parameters/dns:domain/
    ln -s dns:domain /srv/salt/haste/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/haste/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/haste/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``haste`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``haste`` formulas under ``/srv/salt/haste/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/haste/parameters/roles
...
We create a configuration for the role ``haste/server`` in ``/srv/salt/haste/parameters/roles/haste/server.yaml``:
...
We create another configuration for the role ``haste/client`` in ``/srv/salt/haste/parameters/roles/haste/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles haste/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles haste/client
...
.. code-block:: console
    ...
    mv /srv/salt/haste/parameters/dns:domain/  '/srv/salt/haste/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-haste-as-json:
      test.nop:
        - name: {{ haste | json }}
...
Here is an example based on `template-formula/haste/config/file.sls`_:
    ...
    haste-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='haste-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ haste.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``haste`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ haste['winner'] }}
...
.. _template-formula/haste/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/haste/config/file.sls
